...
# UI持ちアイテムなら内容を更新
    execute unless predicate main:social/ui/holding_backpack if entity @e[tag=backpack,distance=..5,tag=open] run function main:social/ui/close
    execute unless predicate main:social/ui/holding_purse if entity @e[tag=purse,distance=..5,tag=open] run function main:social/ui/close
    execute unless predicate main:social/ui/holding_knowledge_book if entity @e[tag=knowledge_book,distance=..5,tag=open] run function main:social/ui/close
# リセット
