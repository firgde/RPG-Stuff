#名前だとか説明文だとか
    ...
    data modify storage item: Item.tag.display.Name set value '{"italic":false,"color":"green","translate":"item.whetstone"}'
    data modify storage item: Item.tag.display.Lore set value ['{"translate":"item.whetstone.lore","color":"white","italic":false}']
    data modify storage item: Item.tag.Customnbt.display.Lore set value ['{"translate":"item.whetstone.lore","color":"white","italic":false}']
    ...
    data modify storage item: Item.tag.CustomModelData set value 2100
...
#ステータスとレア度、耐久値設定
    ...
    scoreboard players set $accbonus buffer -25
...
#実際のステータス
    ...
    data modify storage item: Item.tag.Customnbt.status.acc set value -25
