...
# ダメージ表示
    ...
    execute at @s positioned ~ ~1.5 ~ as @e[sort=nearest,type=item,distance=..0.5] if entity @s[nbt={Item:{components:{"minecraft:custom_data":{DisplayDamage:1b}}}}] run tag @s add dmg_display
    execute as @e[tag=dmg_display,sort=nearest] run data modify entity @s CustomName set from entity @s Item.components."minecraft:item_name"
