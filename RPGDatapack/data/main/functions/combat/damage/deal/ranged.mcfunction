...
# 矢を特定
    execute at @s run tag @e[limit=1,sort=nearest,tag=playerArrow,distance=..5] add arrow.hit
# 属性を取得
...
# デバフデータ取得
    execute store result score $mainElement buff_amount run scoreboard players get @e[limit=1,tag=arrow.hit] buff_amount
    execute store result score $mainElement buff_timer run scoreboard players get @e[limit=1,tag=arrow.hit] buff_timer
# 一番近くの矢からデータ取得
    execute at @s store result score @s damage run scoreboard players get @e[limit=1,tag=arrow.hit] dex
# 最大チャージなら連撃判定のinteraction召喚
    execute at @s if data entity @e[limit=1,tag=arrow.hit] {crit:1b} unless data entity @s Passengers[{Tags:["atk_combo"]}] run summon interaction ~ ~ ~ {Tags:["atk_combo","atk_combo.init"]}
