...
###メインハンドとオフハンドからitem_displayのitem.tagにデータ代入
    # 魔術攻撃はこの時点で属性を定めておく
    data modify entity @s item.components.data.Element.Type append from storage combat: data.magic[0].Element.Type
    data modify entity @s item.components.data.Element.Debuff append from storage combat: data.magic[0].Element.Debuff
    execute unless data storage combat: data.magic[0].Element.Type run data modify entity @s item.components.data.Element.Type append value 0
    execute unless data storage combat: data.magic[0].Element.Type run data modify entity @s item.components.data.Element.Debuff append value {}
    data modify entity @s item.components.data.Element.Type append from storage combat: data.magic[1].Element.Type
    data modify entity @s item.components.data.Element.Debuff append from storage combat: data.magic[1].Element.Debuff
    # componentの属性をスコアに代入
    execute store result score #mainElement buffer run data get entity @s item.components.data.Element.Type[0]
    execute store result score #sideElement buffer run data get entity @s item.components.data.Element.Type[1]
    # 属性を二つ検知したら合成
    ...
    #メインハンドが属性を持っている場合
    execute unless score #mainElement buffer matches 0 if score #sideElement buffer matches 0 run data modify storage buffer: data.Element set from entity @s item.components.data.Element.Type[0]
    #オフハンドが属性を持っている場合
    execute if score #mainElement buffer matches 0 unless score #sideElement buffer matches 0 run data modify storage buffer: data.Element set from entity @s item.components.data.Element.Type[1]
    # 属性が全くなければそのまま一般タグ
