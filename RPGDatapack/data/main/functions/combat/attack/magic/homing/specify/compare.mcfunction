...
# 全候補を実行者とし、thisより優先度が高いならhigherスコアに加算
    ...
    execute as @e[tag=!this,tag=homingCandidate] if score @s homing_priority > @e[limit=1,tag=this] homing_priority run scoreboard players add @e[tag=this] higher 1
