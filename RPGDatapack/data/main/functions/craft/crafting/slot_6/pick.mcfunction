# slot.6のitem_displayからインベントリにアイテムを持ってくる
    ...
    #左クリックしていたらアイテム消去
    execute if entity @s[advancements={main:craft/crafting/slot_6={left=true}}] at @e[type=interaction,tag=crafting,tag=slot.6,limit=1,sort=nearest] run item replace entity @e[limit=1,sort=nearest,tag=slot,type=item_display] container.0 with air
    #右クリックしていたら交換
    execute if entity @s[advancements={main:craft/crafting/slot_6={right=true}}] run function main:craft/crafting/slot_6/place
    #そしてアイテム還元
...
# リセット
    ...
    advancement revoke @s only main:craft/crafting/slot_6
