# slot.1のitem_displayからインベントリにアイテムを持ってくる
    ...
    #左クリックしていたらアイテム消去
    execute if entity @s[advancements={main:craft/alchemy/slot_1={left=true}}] at @e[type=interaction,tag=alchemy,tag=alchemy.slot.1,limit=1,sort=nearest] run item replace entity @e[limit=1,sort=nearest,tag=alchemy.item,type=item_display] contents with air
    #右クリックしていたら交換
