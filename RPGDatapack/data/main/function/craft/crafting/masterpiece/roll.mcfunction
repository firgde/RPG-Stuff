# ((dex * 1.5 + acc + luk) / 6)%の確率で傑作に
    ...
    scoreboard players operation #chance buffer = @a[limit=1] dex
    ...
    scoreboard players operation #chance buffer += @a[limit=1] acc
    scoreboard players operation #chance buffer += @a[limit=1] luk
