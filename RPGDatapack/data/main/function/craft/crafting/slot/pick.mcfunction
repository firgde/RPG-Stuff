# item_displayからインベントリにアイテムを持ってくる
    ...
    #左クリックしていたらアイテム消去
    #スニークしていないなら一個
    execute if entity @p[advancements={main:craft/crafting/interact={left=true}},predicate=!main:sneaking] run item modify entity @n[tag=slot,type=item_display] contents [{"function":"set_count","count":-1,"add":true}]
    ...
    #スニークしているなら全部
    execute if entity @p[advancements={main:craft/crafting/interact={left=true}},predicate=main:sneaking] run item replace entity @n[tag=slot,type=item_display] contents with air
    #右クリックしていたら交換
    #スニークしていないなら一個
    ...
    #そしてアイテム還元
    execute unless score #success_1 buffer matches -2147483648..2147483647 as @p if items entity @s weapon.mainhand * run loot give @s mine 0 -59 0 mainhand
    execute unless score #success_1 buffer matches -2147483648..2147483647 as @p unless items entity @s weapon.mainhand * run item replace entity @s weapon.mainhand from block 0 -59 0 container.0
# リセット
