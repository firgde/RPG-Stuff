# アイテムが一致しているなら加算
    execute if items entity @a[limit=1] weapon.mainhand * store result storage craft:buffer data.id int 1 run data get entity @a[limit=1] SelectedItem.components."minecraft:custom_data".id
