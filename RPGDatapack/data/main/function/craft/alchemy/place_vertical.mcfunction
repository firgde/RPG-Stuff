...
# 水に浮いてるアイテム&interaction
    ...
    execute as @e[tag=alchemy.item,limit=2] run data modify entity @s transformation.scale set value [0.25f,0.25f,0.25f]
    execute as @e[tag=alchemy.item] run scoreboard players set @s mov_direction 1
    execute as @e[tag=alchemy.item,limit=2] at @s run ride @s mount @n[type=interaction,distance=..0.5,tag=alchemy.slot]
