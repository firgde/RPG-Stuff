...
# potionCountに応じてitem_displayの表示を変える
    execute if score #itemCount buffer matches 0 run item replace entity @n[tag=brewing.slot.0] contents with air
    execute if score #itemCount buffer matches ..1 run item replace entity @n[tag=brewing.slot.1] contents with air
    execute if score #itemCount buffer matches ..2 run item replace entity @n[tag=brewing.slot.2] contents with air
# リセット
