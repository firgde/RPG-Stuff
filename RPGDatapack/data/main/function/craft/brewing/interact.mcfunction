# 醸造台に何個アイテムが入っているか見る
    ...
    execute at @n[tag=brewing,type=interaction,distance=..6,scores={hasTarget=1}] store result score #item_count buffer run data get block ~ ~ ~ Items
# 左右どっちでクリックしたかで分岐
    execute if entity @s[advancements={main:craft/brewing/interact={right=true}}] at @n[tag=brewing,distance=..6,scores={hasTarget=1},tag=!brewing.brewed] run function main:craft/brewing/place/generic
