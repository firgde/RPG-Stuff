...
# potionCountに応じてitem_displayの表示を変える
    execute if score #itemCount buffer matches 1.. run item replace entity @n[tag=brewing.slot.0] contents with glass_bottle[item_model="asset:brewing_stand_bottle"]
    execute if score #itemCount buffer matches 2.. run item replace entity @n[tag=brewing.slot.1] contents with glass_bottle[item_model="asset:brewing_stand_bottle"]
    execute if score #itemCount buffer matches 3.. run item replace entity @n[tag=brewing.slot.2] contents with glass_bottle[item_model="asset:brewing_stand_bottle"]
# アイテムを一個減らす
