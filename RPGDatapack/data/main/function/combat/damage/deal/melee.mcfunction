...
# 耐久を減らす
    execute as @a if entity @s[gamemode=!creative] unless items entity @s weapon.mainhand *[custom_data~{Unbreakable:1b}|custom_data~{weaponType:2}] run function items:durability/remove
# からの攻撃力を検出して計算式へ
    #剣の薙ぎ払い
    execute if items entity @a weapon.mainhand *[custom_data~{weaponType:0}] if score @a[limit=1] dealtDamage matches 100..149 at @s run function main:combat/attack/range_attack/sweep
    #ハンマーの衝撃
    execute if items entity @a weapon.mainhand *[custom_data~{weaponType:1}] if score @a[limit=1] dealtDamage matches 100..149 at @s run function main:combat/attack/range_attack/impact
    #短剣の突き
    execute if items entity @a weapon.mainhand *[custom_data~{weaponType:4}] if score @a[limit=1] dealtDamage matches 100..149 at @s run function main:combat/attack/range_attack/stab
...
# ダメージ計算
    ...
    execute if items entity @a weapon.offhand *[custom_data~{offhandOnly:1b}] store result score #side_element buff_amount run data get entity @a[limit=1] equipment.offhand.components."minecraft:custom_data".Element.Debuff.Amount
    ...
    #会心攻撃の処理
    execute as @a if score @s dealtDamage matches 150.. run function main:combat/attack/crit/
    execute as @a run function status:update/
    scoreboard players operation @a atkBuffer = @a atk
    scoreboard players operation @a atkBuffer *= @a dealtDamage
    scoreboard players operation @a atkBuffer /= #100 const
    scoreboard players operation @s damage = @a atkBuffer
