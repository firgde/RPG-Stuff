...
# 属性を取得
    execute store result score #mainElement buffer run data get entity @n[tag=magic] item.components."minecraft:custom_data".Element.Type[0]
    execute store result score #sideElement buffer run data get entity @n[tag=magic] item.components."minecraft:custom_data".Element.Type[1]
...
# デバフデータ取得
    execute store result score #mainElement buff_amount run data get entity @n[tag=magic] item.components."minecraft:custom_data".Element.Debuff[0].Amount
    execute store result score #mainElement buff_timer run data get entity @n[tag=magic] item.components."minecraft:custom_data".Element.Debuff[0].Timer
