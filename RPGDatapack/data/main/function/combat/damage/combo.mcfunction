# this
    tag @s add this
# 攻撃力を持ってくる
    execute unless entity @a[limit=1,advancements={main:combat/attack/combo/sword=false,main:combat/attack/combo/hammer=false,main:combat/attack/combo/dagger=false}] run scoreboard players operation @s damage = @a[limit=1] atk
    execute unless entity @a[limit=1,advancements={main:combat/attack/combo/sword=false,main:combat/attack/combo/hammer=false,main:combat/attack/combo/dagger=false}] run tag @s add hurt.melee
    execute if entity @a[limit=1,advancements={main:combat/attack/combo/wand=true}] run scoreboard players operation @s damage = @a[limit=1] mag
    execute if entity @a[limit=1,advancements={main:combat/attack/combo/wand=true}] run tag @s add hurt.magic
# ダメージ減衰
...
# クリティカル判定抽選
    execute as @a[limit=1] run function main:combat/attack/crit/
    scoreboard players operation @s damage *= @a[limit=1] dealtDamage
