...
# ダメージ表示
    ...
    execute at @s positioned ~ ~1.5 ~ as @n[type=item,distance=..0.5] if entity @s[nbt={Item:{components:{"minecraft:custom_data":{DisplayDamage:1b}}}}] run tag @s add dmg_display
    execute as @n[tag=dmg_display] run data modify entity @s CustomName set from entity @s Item.components."minecraft:item_name"
    execute as @n[tag=dmg_display,distance=..5] run data merge entity @s {PickupDelay:32767,Age:5985,CustomNameVisible:1b}
# リセット
    scoreboard players reset #damage buffer
    tag @n[tag=dmg_display,distance=..5] remove dmg_display
