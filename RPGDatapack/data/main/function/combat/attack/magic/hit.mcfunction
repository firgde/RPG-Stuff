# ふつーの魔術攻撃
# Mob絞り込み
    execute at @e[distance=..50,type=item_display,tag=magic] as @e[team=hostile,tag=!hurt,limit=1,dx=0] if data entity @s {HurtTime:10s} store success score @s has_attacker on attacker if entity @s[type=player]
