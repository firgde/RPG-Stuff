...
# 魔術の種類、属性など
    ...
    execute if items entity @a weapon.offhand *[custom_data~{offhandOnly:1b}] run data modify storage combat: data.magic append from entity @s[tag=verify_cooldown,tag=verify_mana,type=player] equipment.offhand.components."minecraft:custom_data".magicData
    ...
    execute if items entity @a weapon.offhand *[custom_data~{offhandOnly:1b}] run data modify storage combat: data.magic[1].Element set from entity @s[tag=verify_cooldown,tag=verify_mana,type=player] equipment.offhand.components."minecraft:custom_data".Element
# ｻﾓｫｫｫｫﾝ
