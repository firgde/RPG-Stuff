...
# ダメージ&ノックバック再現
    ...
    execute facing entity @a[limit=1] eyes positioned ^-1 ^ ^ run tag @e[team=hostile,distance=..0.7,tag=!this] add hurt.indirect
    scoreboard players operation @e[tag=hurt.indirect] damage = @a atk
    #集中の値によってダメージ減衰する
    scoreboard players operation @e[tag=hurt.indirect] damage *= @a dmg_eff
    ...
    execute as @e[tag=hurt.indirect] at @a run function main:combat/damage/knockback
