...
# 剣の連撃に変更
    execute as @n[tag=atk_combo.target] unless entity @s[tag=atk_combo.sword] run function main:combat/attack/combo/sword/init
# すでに剣なら時間延長
    execute as @n[tag=atk_combo.target] if entity @s[tag=atk_combo.sword] run scoreboard players add @s combo_time 10
# 追加でダメージ与える
...
# 演出
    #1回目
    execute as @n[tag=atk_combo.target] if score @s combo_count matches 1 on vehicle at @s run function main:combat/attack/combo/sword/1
    #2回目
    execute as @n[tag=atk_combo.target] if score @s combo_count matches 2.. on vehicle at @s run function main:combat/attack/combo/sword/2
# リセット
