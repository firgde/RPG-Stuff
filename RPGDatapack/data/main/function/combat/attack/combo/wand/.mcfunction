...
# 杖の連撃に変更
    execute as @n[tag=atk_combo.target] unless entity @s[tag=atk_combo.wand] run function main:combat/attack/combo/wand/init
# 追加でダメージ与える
...
# 演出
    #1回目
    execute as @n[tag=atk_combo.target] if score @s combo_count matches 1.. on vehicle at @s run function main:combat/attack/combo/wand/1
# リセット
