...
# 弓の連撃に変更
    execute as @n[tag=atk_combo.target] unless entity @s[tag=atk_combo.bow] run function main:combat/attack/combo/bow/init
# すでに弓なら時間延長
    execute as @n[tag=atk_combo.target] if entity @s[tag=atk_combo.bow] unless score @s combo_count matches 2 run scoreboard players add @s combo_time 5
# 連撃回数減らす
    scoreboard players add @n[tag=atk_combo.target] combo_count 1
# 演出
    #1回目
    execute as @n[tag=atk_combo.target] if score @s combo_count matches 1 as @p at @s run function main:combat/attack/combo/bow/1
    #2回目
    execute as @n[tag=atk_combo.target] if score @s combo_count matches 2 as @p at @s run function main:combat/attack/combo/bow/2
