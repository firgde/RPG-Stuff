...
# 攻撃力と属性を設定
    #ダメージ減衰
    scoreboard players operation @s dex = @a[limit=1] dex
    ...
    execute if items entity @a[limit=1] weapon.offhand *[custom_data~{offhandOnly:1b}] store result score #side_element buffer run data get entity @a[limit=1] equipment.offhand.components."minecraft:custom_data".Element.Type
