# シュルカーボックスにアイテムを避難させた後、棒を入れる
    ...
    item replace entity @s container.21 with stick[custom_data={ui:1b,menu:"backpack"},tooltip_display={hide_tooltip:true},item_model="air"]
# ページがマッチするならアクション
    execute if score @s page matches 2 unless score @s sub_page matches 0 run function main:social/backpack/use/page/2_withdraw/prev
