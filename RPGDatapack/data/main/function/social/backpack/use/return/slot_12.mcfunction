# シュルカーボックスにアイテムを避難させた後、棒を入れる
    ...
    item replace entity @s container.12 with stick[custom_data={ui:1b,menu:"backpack"},tooltip_display={hide_tooltip:true},item_model="air"]
# ページがマッチするならアクション
    ...
    execute if score @s page matches 3 run item replace entity @s container.12 with stick[custom_data={ui:1b,menu:"backpack",button:1b},item_model="asset:item_remove",item_name={"translate":"social.ui.purse.count.-1"}] 1
