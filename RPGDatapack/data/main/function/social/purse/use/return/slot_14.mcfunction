# シュルカーボックスにアイテムを避難させた後、棒を入れる
    ...
    execute if score @s sub_page matches 1 run item replace entity @s container.14 with stick[custom_data={ui:1b,menu:"purse"},custom_model_data={floats:[8997]},item_name={"translate":"social.ui.purse.count.1","italic": false}] 1
    execute if score @s page matches 2 if score @s sub_page matches 0 run scoreboard players set #moneyamount buffer 20
    execute if score @s sub_page matches 1 run scoreboard players add #moneycount buffer 1
    execute if score @s page matches 2 run function main:social/purse/use/page/2.1_count
    execute if score @s page matches 0 run item replace entity @s container.14 with stick[custom_data={ui:1b,menu:"purse"},tooltip_display={hide_tooltip:true},item_model="air"]
    #表示額
