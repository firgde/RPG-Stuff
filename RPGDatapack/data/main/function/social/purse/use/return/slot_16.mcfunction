# シュルカーボックスにアイテムを避難させた後、棒を入れる
    ...
    execute if score @s sub_page matches 1 run item replace entity @s container.16 with stick[custom_data={ui:1b,menu:"purse"},item_model="asset:gemstone_shard_add",item_name={"translate":"social.ui.purse.count.8","italic": false}] 8
    execute if score @s sub_page matches 1 run scoreboard players add #moneycount buffer 8
    execute unless score @s sub_page matches 1 run item replace entity @s container.16 with stick[custom_data={ui:1b,menu:"purse"},tooltip_display={hide_tooltip:true},item_model="air"]
    #表示額
