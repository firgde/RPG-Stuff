# マクロでスロットにアイテム配置
    ...
    function main:social/knowledge_book/use/page/5_status_explore/place with storage temp: temp
# 1足す
    scoreboard players add #index buffer 1
# 再帰
    scoreboard players add #rec buffer 1
    execute if score #rec buffer matches ..17 run function main:social/knowledge_book/use/page/5_status_explore/rec
