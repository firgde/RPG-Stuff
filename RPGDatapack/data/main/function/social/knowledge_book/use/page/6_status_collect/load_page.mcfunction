## slot 0~17をモブ情報で埋める
    ...
    data modify storage temp: data.collect set from storage book: data.collect
    scoreboard players operation #rec buffer = @s sub_page
    scoreboard players operation #rec buffer *= #18 const
    execute unless score @s sub_page matches 0 run function main:social/knowledge_book/use/page/6_status_collect/remove_data
    ...
    function main:social/knowledge_book/use/page/6_status_collect/rec
# 矢印配置
    ...
    execute if data storage temp: data.collect.item[18] run item replace entity @s container.23 with stick[custom_data={ui:1b,menu:"knowledge_book"},item_model="asset:next",item_name={"translate": "social.ui.knowledge_book.next","italic": false}]
