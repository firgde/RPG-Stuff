# ストレージからデータ取得
    ...
    $data modify storage item: Item set from storage temp: data.collect.item[$(index)]
...
# item_modifierを適用
    $item modify entity @s container.$(slot) [{"function":"set_name","entity":"this","target":"item_name","name":{"nbt":"data.collect.item[$(index)].components.\"minecraft:item_name\"","storage":"temp:","interpret":true}},{"function":"set_custom_model_data","floats":{values:[{"type":"storage","storage":"temp:","path":"data.collect.item[$(index)].components.\"minecraft:custom_model_data\".floats[0]"}],"mode": append}}]
